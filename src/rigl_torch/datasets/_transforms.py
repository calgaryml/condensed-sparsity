...
class RandomMixup(torch.nn.Module):
    ...
    Args:
        ...
        p (float): probability of the batch being transformed. Default value is
        0.5.
    ...
    def forward(self, batch: Tensor, target: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # It's faster to roll the batch by one instead of shuffling it to create
        # image pairs
...
class RandomCutmix(torch.nn.Module):
    ...
    def forward(self, batch: Tensor, target: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        # Implemented as on cutmix paper, page 12 (with minor corrections on
        # typos).
        lambda_param = float(
            torch._sample_dirichlet(torch.tensor([self.alpha, self.alpha]))[0]
        )
