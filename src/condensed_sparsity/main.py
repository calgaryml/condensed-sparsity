...
if __name__ == "__main__":
    ...
    # === Dataset
    parser.add_argument(
        "--dataset-dir", type=str, help="dataset directory", default="./data"
    )
    parser.add_argument(
        "--dataset", type=str, help="dataset to train with", default="MNIST"
    )
    parser.add_argument(
        "--normalize-pixelwise",
        default=False,
        action="store_true",
        help="do pixelwise normalization of the data",
    )
    parser.add_argument(
        "--train-subset-size",
        type=int,
        help="Subset of dataset to use, 0 indicates use full dataset",
        default=0,
    )

    # === Training
    parser.add_argument(
        "--max-epochs", type=int, help="maximum training epochs", default=3
    )
    parser.add_argument("--lr", type=float, help="learning rate", default=0.01)
    parser.add_argument(
        "--lr-decay", type=str, help="learning rate decay type", default="none"
    )
    ...
    parser.add_argument(
        ...
        help="model type",
        default="Net",
        choices=["Net", "SparseNet", "CondNet"],
    )
    parser.add_argument(
        "--no-bias", default=False, action="store_true", help="don't use biases"
    )
    parser.add_argument(
        "--num-layers", type=int, help="total number of layers", default=1
    )
    parser.add_argument(
        "--num-mid", type=int, help="width of the hidden layer(s)", default=0
    )
    parser.add_argument(
        "--fan-in", type=int, help="fan-in for condLayer", default=0
    )
    parser.add_argument(
        "--fan-out-const",
        type=str,
        default="True",
        choices=["True", "False"],
        help="use same fan-out for all units in sparse (condensed) layer",
    )
    ...
    parser.add_argument(
        ...
        default="True",
        choices=["True", "False"],
        help="disable non-linear (ReLU) activation functions",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        choices=["none", "per_neuron", "per_layer"],
        help="sparsity type for SparseNet",
    )
    parser.add_argument(
        "--connect-type",
        type=str,
        default="none",
        choices=["none", "scattered", "block"],
        help="connectivity type for SparseNet",
    )
    ...
    parser.add_argument(
        "--chkpt-epochs",
        type=int,
        help="Save checkpoint, frequency in epochs. Do not save if zero.",
        default=0,
    )
    ...
    (
        ...
        dataset, dataset_dir, train_batch_size, test_batch_size, no_da, kwargs
    )
    ...
    patience = 20
    test_acc_tracker = list(
        np.zeros(2 * patience)
    )  # keep a list of test acc over past some eps
    model.train()
    ...
    if train_subset_size > 0:
        ...
        train_loader = torch.utils.data.DataLoader(
            new_train_set, batch_size=batch_size, shuffle=True
        )
    ...
    while epoch <= max_epochs:
        ...
        for mbi, (images, labels) in enumerate(train_loader):
            ...
            if normalize_pixelwise:
                images = pixelwise_normalization(images)
        ...
        writer.add_scalars(
            "loss",
            {"test": test_loss, "train": train_loss},
            global_step=step_counter,
            walltime=time.time() - start_time,
        )
