...
def make_outputdirname(args):
    ...
    if args.train_subset_size>0: output_dir+=f'_train_on_{args.train_subset_size}_samples'
    if args.normalize_pixelwise: output_dir+=f'_pixelwise_normalization'

    output_dir+=f'_LR_{args.lr}'
    if args.lr_decay!='none': output_dir+=f'_{args.lr_decay}_LRdecay'
...
def make_smask(dims, fan_in, sparsity_type, connect_type, fan_out_const):
    ...
    if sparsity_type=='per_neuron':
        ...
        if connect_type=='scattered':
            if fan_out_const:
                sys.exit(f'sparsity_type {sparsity_type} connect_type {connect_type} >>> fan_out_const does not work yet! TO BE DONE!')
            else:
                ...
                smask.zero_()
                smask[torch.arange(dim_out)[:, None], k_idx] = 1 # setting to 1 weights to be removed
        ...
        #================
            ...
            if fan_out_const:
                ...
                k_idx= torch.randint(low=0,high=dim_in-fan_in+1,size=(dim_out,1))
                # add subsequent indices to cover full block
                k_idx= k_idx.repeat(1,fan_in)
                for i in range(fan_in): k_idx[:,i]+=i

                smask.fill_(1) # setting 1s everywhere
                smask[torch.arange(dim_out)[:, None], k_idx] = 0 # setting to 0 where weights to keep
    #
    #=======================
    ...
    print(f'Applying smask to layer will freeze {s} of {np.prod(dims)} weights (= {p:.2f}% sparse).')
...
def load_dataset(dataset, dataset_dir, batch_size, test_batch_size, no_da, kwargs):
    if dataset=='MNIST':
        ...
        train_loader = load_FashionMNIST(data_dir=dataset_dir, split='train', batch_size=batch_size, shuffle=True, kwargs=kwargs)
        test_loader = load_FashionMNIST(data_dir=dataset_dir, split='test', batch_size=test_batch_size, shuffle=False, kwargs=kwargs)
        input_size= 28**2
        num_classes= 10
        num_channels= 1
    elif dataset=='cifar100_resnet50':
        train_loader = load_cifar100_resnet50(data_dir=dataset_dir, split='train', batch_size=batch_size, shuffle=True, kwargs=kwargs)
        test_loader = load_cifar100_resnet50(data_dir=dataset_dir, split='test', batch_size=test_batch_size, shuffle=False, kwargs=kwargs)
        input_size  = 2048
        num_classes = 100
        num_channels= 1
    elif dataset=='svhn_resnet18':
        train_loader = load_svhn_resnet18(data_dir=dataset_dir, split='train', batch_size=batch_size, shuffle=False, kwargs=kwargs)
        test_loader = load_svhn_resnet18(data_dir=dataset_dir, split='test', batch_size=test_batch_size, shuffle=False, kwargs=kwargs)
        input_size  = 512
        num_classes = 10
        num_channels= 1
    else:
...
def load_SVHN(data_dir, split, batch_size, shuffle, kwargs):
    ...
    dataset = datasets.SVHN(root=data_dir, split=split, transform=transform, download=True)
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, **kwargs)
    return data_loader
...
def load_svhn_resnet18(data_dir, split, batch_size, shuffle, kwargs):
    ...
    dataset = cu_datasets.svhn_resnet18(root='./data', train=train_flag, transform=None)
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, **kwargs)
    return data_loader
...
def load_cifar100_resnet50(data_dir, split, batch_size, shuffle, kwargs):
    ...
    dataset = cu_datasets.cifar100_resnet50(root='./data', train=train_flag, transform=None)
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, **kwargs)
    return data_loader
...
def evaluate(model, emb, data_loader, normalize_pixelwise, input_size, device, criterion):
    """ Evaluate model, return prediction accuracy and loss. """
    model.eval()
    if emb: emb.eval()
        ...
        for i, (inputs, labels) in enumerate(data_loader):
            ...
            if emb:
                ...
                if num_channels==1:
                    ...
                    inputs= inputs.to(device)
                if normalize_pixelwise: inputs= pixelwise_normalization(inputs)
...
def load_dataset_TL(dataset, batch_size, test_batch_size, img_size, kwargs):
    ...
    if dataset=='CIFAR100': # split, batch_size, shuffle, img_size, kwargs
        train_loader= load_CIFAR100(split='train', batch_size=batch_size, shuffle=True, img_size=img_size, kwargs=kwargs)
        test_loader= load_CIFAR100(split='test', batch_size=test_batch_size, shuffle=False, img_size=img_size, kwargs=kwargs)
        num_channels= 3
        num_classes = 100
    else:
        print(f'Error: Dataset {dataset} not available!')
